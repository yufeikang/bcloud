...
_ = Config._
from bcloud import gutil, pcs, util
    ...
    def request_data(self):
        ...
        def on_tasks_received(info, error=None):
            if error or not info:
                logger.error(
                    "BTBrowserDialog.on_tasks_received: %s, %s." % (info, error)
                )
                return
            ...
            for task in tasks:
                ...
                select = size > MIN_SIZE_TO_CHECK or task["file_name"].endswith(
                    CHECK_EXT
                )

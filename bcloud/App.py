...
Config.check_first()
_ = Config._
from bcloud import const, gutil, util
...
# 用于处理拖放上传
DROP_TARGETS = ((TargetType.URI_LIST, Gtk.TargetFlags.OTHER_APP, TargetInfo.URI_LIST),)
...
class App:
    ...
    def on_app_startup(self, app):
        ...
        self.window.drag_dest_set(
            Gtk.DestDefaults.ALL, DROP_TARGET_LIST, Gdk.DragAction.COPY
        )
        self.window.connect(
        ...
        preferences_action.connect("activate", self.on_preferences_action_activated)
        ...
        self.window.add_accelerator(
            "activate-default", self.accel_group, key, mod, Gtk.AccelFlags.VISIBLE
        )
    ...
    def on_main_window_drag_data_received(
        self, window, drag_context, x, y, data, info, time
    ):
        """从其它程序拖放目录/文件, 以便上传.
    ...
    def init_status_icon(self):
        ...
        pause_upload_item = Gtk.MenuItem.new_with_label(_("Pause Upload Tasks"))
        pause_upload_item.connect(
            "activate", lambda item: self.upload_page.pause_tasks()
        )
        menu.append(pause_upload_item)

        pause_download_item = Gtk.MenuItem.new_with_label(_("Pause Download Tasks"))
    ...
    def init_notify(self):
        ...
        if self.profile["use-notify"]:
            ...
            if not status:
                return
            self.notify = Notify.Notification.new(Config.APPNAME, "", Config.NAME)
